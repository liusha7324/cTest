/*"***************************************************************"*/
/*"国网方案CPU卡数据结构                                          "*/
/*"参考文件：智能电能表信息交换安全认证技术规范(IC卡)0626.doc     "*/
/*"刘新润                                                         "*/
/*"日期：2009年7月27日                                            "*/
/*"***************************************************************"*/
//------------------------------------------------------------------
...
struct Triff_Data
	{
	unsigned long Triff[32];        /*" 费率1-32"*/
	unsigned long Grads_Value[6];   		/*" 阶梯值1-6"*/
	unsigned long Grads_Price[7];   		/*" 阶梯电价1-7"*/
	unsigned char Bill_Date[4][3];	    		/*" 年结算日 "*/
	unsigned char Grad_Switch_Date[5];	 /*"两套阶梯切换时间"*/	
	};
#define LENGTH_TRIFF_DATA  sizeof(struct Triff_Data)
#define TRIFF1_OFF				4
#define GRADE1_OFF              		(TRIFF1_OFF + 32*4)
#define GRADE1_PRICE_OFF        	(GRADE1_OFF + 6*4)
#define BILL_DATE_OFF           		(GRADE1_PRICE_OFF + 7*4)
#define GRADE_SWITCH_OFF        	(BILL_DATE_OFF + 12)      
...
#define LENGTH_MONEYBAG_DATA  sizeof(struct Moneybag_Data)
	...
	#define BUY_COUNT_CPU_ESAM				REMAIN_MONEY_CPU_ESAM+4
